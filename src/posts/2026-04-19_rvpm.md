...
tags: ["rust", "neovim", "plugin-manager", "cli", "tui"]
...
以前、Deno + denops ベースの [dvpm](https://github.com/yukimemi/dvpm) を作り、愛用していました（[過去の記事](/posts/2023-06-09_dvpm/)・[キャッシュ機能の記事](/posts/2026-01-12_dvpm_lazy_cache/)）。dvpm は denops との親和性も高く気に入っていたのですが、使い続けるうちにいくつか気になる点が出てきました。
...
TOML という宣言的なフォーマットに寄せつつ、`{% if %}` による条件分岐や `{{ vars.xxx }}` / `{{ env.XXX }}` / `{{ is_windows }}` での変数展開が挟めるので、「設定は宣言的に、でもちょっとしたロジックは書きたい」という欲張りな要求を綺麗に満たせています。dvpm 時代に TypeScript だから自由にできていた部分を、TOML に落としても失わずに済んだ形です。
