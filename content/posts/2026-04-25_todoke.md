...
description: 入力されたファイルパス・URL・任意の文字列を TOML のルールに従って、適切なエディタやスクリプトに「届け」る Rust 製ディスパッチャ todoke を作りました。$EDITOR や OS の既定プログラムとして使うのが想定用途です。
taxonomies:
  tags: ["cli", "editor", "neovim", "rust", "windows"]
extra:
  type: post
---
...
- **Tera テンプレート対応** — `command` / `listen` / `args` / `group` などすべての値で {% raw %}`{% if is_windows() %}`{% endraw %} や {% raw %}`{{ env.HOME }}`{% endraw %} が使える
...
さらに `group` を使えば、ひとつの nvim にすべて流すのではなく、**用途別に複数の nvim インスタンスを使い分ける**こともできます。`listen` の中に {% raw %}`{{ group }}`{% endraw %} を埋め込んでおけば、group が変わるたびに別の pipe / socket になって、別の nvim プロセスとして起動・再利用される、という仕組みです。
...
{% raw %}`{{ passthrough }}`{% endraw %} を `args` の中で参照すると、フラグを **任意の位置に挿入**できます。gvim のように `--remote-silent <file>` の前にフラグを置きたい、というケースに便利です。
...
{% raw %}`{{ passthrough }}`{% endraw %} を**単独の args 要素**として書いた場合は **inline 展開**されるので、`-c :set ft=md` のような複数 argv のフラグも 1 つの `""` にまとめられず、ちゃんと argv 単位で渡ります。
...
の 3 通りに切り替わります。{% raw %}`{% if %}`{% endraw %} の中で **`[todoke.gui.args]` テーブルそのものを条件包含できる**のは、todoke が TOML パース前に全文を Tera に通している (= TOML 構造ごと条件分岐できる) ことの利点です。
...
- {% raw %}`listen = '{% if is_windows() %}\\.\pipe\...{% else %}/tmp/...{% endif %}'`{% endraw %} のような OS 分岐
- {% raw %}`command = "{{ vars.gui }}"`{% endraw %} のような変数経由の参照 ([vars] セクションで一括切り替え)
- {% raw %}`group = "{{ env.PROJECT_GROUP | default(value='default') }}"`{% endraw %} のような環境変数経由の動的グループ

がすべて宣言的な設定だけで書けます。

それに加えて、**入力種別ごとに使える変数**も用意されています。

- **file 入力**: {% raw %}`{{ file_path }}`{% endraw %} (正規化された絶対パス) / {% raw %}`{{ file_dir }}`{% endraw %} / {% raw %}`{{ file_name }}`{% endraw %} / {% raw %}`{{ file_stem }}`{% endraw %} / {% raw %}`{{ file_ext }}`{% endraw %} (拡張子、ドットなし)
- **URL 入力**: {% raw %}`{{ url_scheme }}`{% endraw %} / {% raw %}`{{ url_host }}`{% endraw %} / {% raw %}`{{ url_port }}`{% endraw %} / {% raw %}`{{ url_path }}`{% endraw %} / {% raw %}`{{ url_query }}`{% endraw %} / {% raw %}`{{ url_fragment }}`{% endraw %}
- 共通: {% raw %}`{{ input }}`{% endraw %} (生の入力文字列) と {% raw %}`{{ input_type }}`{% endraw %}、ルールの `match` で取った正規表現キャプチャの {% raw %}`{{ cap.1 }}`{% endraw %} / {% raw %}`{{ cap.<name> }}`{% endraw %}
...
`exec` ターゲットの `args` に {% raw %}`{{ input }}`{% endraw %} や {% raw %}`{{ file_path }}`{% endraw %} が出てくると、todoke はそれを検知して**末尾への自動 append を抑制**します。{% raw %}`{{ passthrough }}`{% endraw %} についても同様です。これは設定が直感的に書けるようにするための小さな仕掛けで、たとえば
