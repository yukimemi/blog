...
description: CLI ファーストで `loader.lua` を事前コンパイルする Rust 製 Neovim プラグインマネージャー rvpm を作りました。dvpm からの移行先として、遅延ロードと起動速度の両立を狙っています。
taxonomies:
  tags: ["ai", "claude-code", "cli", "neovim", "plugin-manager", "rust", "tui"]
extra:
  type: post
---
...
以前、Deno + denops ベースの [dvpm](https://github.com/yukimemi/dvpm) を作り、愛用していました（[過去の記事](/posts/dvpm/)・[キャッシュ機能の記事](/posts/dvpm_lazy_cache/)）。dvpm は denops との親和性も高く気に入っていたのですが、使い続けるうちにいくつか気になる点が出てきました。
...
{% raw %}`{% if %}`{% endraw %} は「そもそも `loader.lua` に含めない」、`cond = "..."` は「含めるが実行時に Lua で判定」、という使い分けになっています。
...
![rvpm AI add](/static/images/2026-04-19_rvpm_ai_add.png)
...
TOML という宣言的なフォーマットに寄せつつ、{% raw %}`{% if %}`{% endraw %} による条件分岐や {% raw %}`{{ vars.xxx }}`{% endraw %} / {% raw %}`{{ env.XXX }}`{% endraw %} / {% raw %}`{{ is_windows }}`{% endraw %} での変数展開が挟めるので、「設定は宣言的に、でもちょっとしたロジックは書きたい」という欲張りな要求を満たせています。dvpm 時代に TypeScript だから自由にできていた部分を、TOML に落としても失わずに済んだ形です。
