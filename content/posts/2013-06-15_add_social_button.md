...
taxonomies:
  tags: ["button", "python", "sns", "tinkerer"]
extra:
  type: post
---
...
{% raw %}`{{ self.buttons() }}`{% endraw %} という形 で参照している。
...
の {% raw %}`{% block extrahead %}`{% endraw %} に追記する。
...
URLやタイトルをそのページにあったものに変換している。 {% raw %}`{{ pagename }}`{% endraw %}
、 {% raw %}`{{ file_suffix }}`{% endraw %} などは ググればわかる。
