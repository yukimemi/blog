...
description: Alfred や Raycast のようなキーボードドリブンなランチャー shun を Rust + Tauri + Svelte で作りました。
taxonomies:
  tags: ["launcher", "linux", "macos", "rust", "svelte", "tauri", "windows"]
extra:
  type: post
---

> [!NOTE] Update (2026-04-04)
...
![Edge で Bing を開く](/static/images/shun-args-edge-bing.png)

![Chrome で Google を開く](/static/images/shun-args-chrome-google.png)
...
- `%APPDATA%` や `$HOME`, `${XDG_CONFIG_HOME}` などの**環境変数を含むパス**を直接入力 → 展開して開く
...
`/save` や `/config` などのコマンドは、Tab キーを押すことで引数の候補（設定項目やファイル名）がドロップダウン表示されます。
...
`path` や `args` には [Tera](https://keats.github.io/tera/) テンプレート構文が使えます。Tab で Args モードに入って入力した内容が {% raw %}`{{ args }}`{% endraw %}（文字列全体）や {% raw %}`{{ args_list }}`{% endraw %}（スペース区切りの配列）に展開されます。

利用可能な変数・関数：
- {% raw %}`{{ args }}`{% endraw %}: 入力内容全体（文字列）
- {% raw %}`{{ args_list.0 }}`{% endraw %}: スペース区切りの第1引数（0番目）
- {% raw %}`{{ env.VAR_NAME }}`{% endraw %}: 環境変数
- {% raw %}`{{ vars.my_var }}`{% endraw %}: `[vars]` セクションで定義した変数
- {% raw %}`{{ file_path }}`{% endraw %}: `overrides` で差し替えられた元のファイルパス（フルパス）
- {% raw %}`{{ now() | date(format="%Y%m%d") }}`{% endraw %}: 現在の日時
- {% raw %}`{{ args | urlencode }}`{% endraw %}: URL エンコードフィルタ
...
{% raw %}`{{ env.VAR_NAME }}`{% endraw %} または Tera 標準の {% raw %}`{{ get_env(name="VAR") }}`{% endraw %} で環境変数を参照できます。
...
`[vars]` セクションに任意のキーを定義すると、`path` や `args` の Tera テンプレートから {% raw %}`{{ vars.キー名 }}`{% endraw %} で参照できます。`config.toml` と `config.local.toml` はマージしてから展開されるので、`config.local.toml` 側の `[vars]` に書いた値を `config.toml` の `[[apps]]` で使うことができます。マシンごとに異なるパスなどを `config.local.toml` にだけ書いておく運用が便利です。
